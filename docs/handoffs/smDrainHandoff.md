...
## Session 23 - the referee had already ruled; the receipt never landed

Session 23 ran `complete` as its first act (refused, `open-needs-ceo` #131, fourth in a row) and
then did the one thing three previous innings had not: **it stopped measuring the queue and read
the consumer.**

### The check that inverted the diagnosis

Sessions 21 and 22 built a genuinely good latency model — a ~25 min median, correctly re-scoped as
a *within-burst* statistic, plus the lesson's own skip test ("has anything opened after mine been
ruled while mine sat?"). Session 23 re-ran that test and it *fired* for the first time:

    132|smDrainFlowers|RULED|2026-09-04T04:02:24Z|2026-09-04T04:02:24Z|ceo
    131|smDrainHandoff|OPEN |2026-09-04T03:32:28Z|                    |

#132 was opened *and* ruled 30 minutes after #131, by the CEO, while #131 sat. Under session 22's
own stated bar that is "the first real evidence of a skip". It is not a skip. It is the tell that
the CEO **thought it had already ruled #131** — which one `grep 131 DAY-LOG.md` confirmed.

The lesson, banked global as `2026-09-04-ruling-sits-open-too-long-check`: *before modelling a
queue's service time, grep the consumer's OUTPUT for your id.* A stall and a lost receipt are
indistinguishable from the queue side. Three innings of correct arithmetic on the wrong hypothesis
is what that costs.

### One hypothesis raised and REFUTED — do not re-raise it

Session 23 first suspected desk-check.sh:467 was a **false green** (`${nc:-0}` defaulting to 0
because `nc` was only set on the `in` path). It read the file: the assignment at line 166 sits in
an unguarded block that runs in mid, in and out. **The check is live and it went red correctly.**
The premise was verified before it was acted on — which is, with some irony, exactly the standing
instruction ruling #132 issued the same hour ("verify the premise of an inherited fallback before
executing it").

### State at end of session 23

- 17/17 closed, ruler GREEN, rc 0. Nothing about the WORK is owed.
- `complete` refused, fence 23, `open-needs-ceo` #131.
- **#133 OPEN (needs-ceo)** — the transcription request. This is the whole remaining act.
- Nothing was committed to `main` this inning except this handoff. No code was touched: every
  fix this inning identified lives in `ceo-desk` or `pitching-machine`, neither of which is in
  this lane's frozen manifest.
