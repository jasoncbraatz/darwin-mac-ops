...
branch: "main"
gh_sha: "2a4039ad04a342e8a12fb1ddc8191c01dbea05a4"
updated: "2026-09-04"
...
lessons_consulted: ["2026-08-28-rail-project-blocked-open-ruling-many", "2026-08-28-abridge-py-sets-ceo-only-decision"]
lessons_banked: ["2026-09-04-ruling-channel-s-median-time-rule", "2026-09-04-open-ceo-ruling-can-overtaken-events"]
live_theme: "THE REFEREE IS NOT FAST, IT IS EPISODIC -- AND #131 IS MOOT. Session 21 concluded 'healthy fast referee, ~25 min median, wait'. Session 22 applied the banked global lesson 2026-08-28-rail-project-blocked-open-ruling-many, whose test is: has anything opened AFTER yours been RULED while yours sat? The answer is NO -- #131 is the NEWEST ruling in the estate and nothing has jumped it. But grouping by DISTINCT ruled_at exposes what the median hides: the CEO does not serve continuously, it wakes in BURSTS (2026-09-03T19:05 through 2026-09-04T00:43, inter-wake gaps 43-70 min, 1-4 rulings per wake) separated by silences of hours to DAYS -- the record contains a 2026-09-01T08:41 -> 2026-09-03T11:05 gap of ~50 HOURS. #131 opened 03:32:28Z, which is 2h49m AFTER the last burst ended, so the ~25 min median is a WITHIN-BURST statistic and does not predict this wait at all. At 26 minutes old and 3h15m past the last CEO wake, #131 is still NOT LATE against a channel that routinely goes quiet for 50 h. SECOND AND MORE USEFUL FINDING: #131 IS MOOT ON ITS FACE -- it asks whether card 1218153310094177 should close on its own two items, and that card is ALREADY CLOSED, the manifest is 17/17, verify-smdrain.sh handoff is rc 0. The CEO is being asked to ratify an action already taken. WAITING REMAINS CORRECT. Do not escalate, do not duplicate, do not rule."
phase: "COMPLETE, PENDING ONE REFEREE CALL THAT IS MOOT BUT NOT YET LATE. 17 frozen cards, **17 closed**, every one with a bb-close receipt. `verify-smdrain.sh handoff` -> 'RULER GREEN', rc 0, re-run and re-confirmed in session 22 (runtime ~1 s, so there is no tail risk on `complete`). `rail.py complete --project smDrainHandoff --fence 22 --allow-dirty '<sibling files>'` was RUN as the FIRST act of session 22 and REFUSED, correctly, reason `open-needs-ceo`, ruling #131. Three sessions have now run it and been refused; the refusal is the rail working, not a defect. What session 22 adds is (a) the consumer-cadence measurement that stops the next session raising a false alarm at 3x median, and (b) the observation that #131's question is already answered by the closed card. **The only remaining act is a CEO ruling on #131, and it is a one-line ruling.**"
gate_passed: true
next_at_bat: "**RUN `rail.py complete` AS THE FIRST ACT OF THE INNING. Nothing else is owed.** `python3 ~/repos/pitching-machine/rail.py complete --project smDrainHandoff --fence <n> --allow-dirty 'repo: checkout carries a sibling lane modified docs/HANDOFF.md and an untracked launchagents/com.braatz.flowers-bounce-watchdog.plist; neither is this lane work'`. verify_cmd is ~1 s, so run it immediately and unconditionally. If it prints `ok: true` the project is CLOSED -- write two sentences and stop. **If it still prints `open-needs-ceo`, STOP THERE. Sessions 20, 21 and 22 have each spent a full inning re-deriving this and the answer has not changed.** Do NOT re-run the ledger queries; sessions 21 and 22 already ran the two that matter and both are written up below. Do NOT open a second ruling (abridge is open|rule|list|show, there is no escalate and no withdraw verb, so a duplicate is noise, not escalation -- banked 2026-09-04-rail-py-complete-s-open-ceo). Do NOT rule it yourself; it is needs_ceo=1 and your tier may not. **The ONE thing worth a command is the cadence test, and only if #131 is by then more than a DAY old:** `sqlite3 ~/.local/state/auto-bridge/ledger.db \"SELECT ruled_at, ruled_by, count(*) FROM rulings WHERE status='RULED' AND ruled_at IS NOT NULL GROUP BY ruled_at ORDER BY ruled_at DESC LIMIT 14\"` -- if a CEO BURST has occurred since #131 opened and #131 was NOT in it, that is the first real evidence of a skip and is worth writing down. Under that bar, waiting is the measured action: say so in one sentence and stop WITHOUT rewriting this file. **NOTE the ledger column is `project_id`, NOT `project`.** Two items remain teed up for OTHER lanes, neither in this frozen manifest: (1) rail.py ruler freeze records ZERO files for `~`-prefixed verify_cmds (`ruler show --project smDrainHandoff` -> files_json `{}`), root-caused to `ruler_digest()` at rail.py ~2038 not calling expanduser while a sibling at ~2709 does; fix surface pitching-machine, needs its own card and probably a CEO ruler-amend per project. (2) `rail.py complete`'s own refusal text says 'Get the ruling (or withdraw it)' and **there is no withdraw verb**, so a worker tier has zero legal moves on its own blocking call; that dead instruction has now cost three sessions an inning each. Banked as 2026-09-04-rail-py-complete-s-open-ceo. **USE `command grep`, not `grep`.**"
blockers: []
drift_flags: []
...
`rail.py complete` has now been RUN and REFUSED in three consecutive innings (fences 20, 21, 22):
...
ruling on #131 — and #131's question is MOOT: it asks whether card 1218153310094177 should close
on its own two items *or keep waiting on a global colour it does not own*, and that card is
**already closed** with the manifest at 17/17 and the ruler green. A CEO can rule it in one
command, after which `rail.py complete` should pass on the first try (verify_cmd is ~1 s, no tail
risk). Session 22 also measured that the referee is EPISODIC rather than slow, so #131 being open
for hours is expected and is not evidence of a stall — see **Session 22** below before concluding
...
## Session 22 - the median was a within-burst statistic, and #131 is moot

Session 22 ran `complete` as its first act, as instructed. It was refused with `open-needs-ceo`
#131 for the third session running. Everything below is what that inning added; **the next worker
should not re-derive any of it.**

### 1. The lesson's own test: nothing has jumped #131

The banked global lesson `2026-08-28-rail-project-blocked-open-ruling-many` gives a sharper test
than "how old is it": *before concluding 'slow queue, keep waiting', check whether LATER-opened
rulings have been RULED while yours sat.* Applied (used, this inning):

    id   project           status  opened_at             ruled_at              by
    128  smDrainGateAttrib RULED   2026-09-03T23:34:03Z  2026-09-04T00:00:50Z  ceo
    129  smDrainN8n        RULED   2026-09-03T23:43:59Z  2026-09-04T00:00:50Z  ceo
    130  smDrainGateAttrib RULED   2026-09-04T00:18:01Z  2026-09-04T00:43:49Z  ceo
    131  smDrainHandoff    OPEN    2026-09-04T03:32:28Z  -                     -

**#131 is the newest ruling in the estate.** Nothing opened after it, so nothing has been served
ahead of it. By that lesson's own criterion this is a queue that has not reached you yet, not a
channel that is skipping you — and waiting is correct.

### 2. What the ~25 min median was actually measuring

Session 21's median is real but it answers the wrong question. Group the ruled rows by DISTINCT
`ruled_at` and the CEO's shape appears:

    2026-09-04T00:43:49Z  ceo  1     <-- LAST WAKE. Nothing since.
    2026-09-04T00:00:50Z  ceo  2
    2026-09-03T23:16:02Z  ceo  4
    2026-09-03T22:30:43Z  ceo  1
    2026-09-03T21:48:36Z  ceo  2
    2026-09-03T20:37:57Z  ceo  2
    2026-09-03T20:29:02Z  ceo  1
    2026-09-03T19:06:37Z  ceo  1
    2026-09-03T19:05:38Z  ceo  1
    2026-09-03T16:13:53Z  gm   1
    2026-09-03T13:22:37Z  ceo  1
    2026-09-03T11:56:33Z  ceo  1
    2026-09-03T11:05:38Z  ceo  1
    2026-09-01T08:41:08Z  ceo  2     <-- ~50 HOUR gap to the row above it

**The CEO does not serve continuously; it wakes in bursts and rules 1-4 calls per wake.** Inside
the 2026-09-03 evening burst the inter-wake gap is 43-70 min, which is where session 21's ~25 min
median comes from. Between bursts the silence runs to hours, and once in this very record to
**~50 hours**.

#131 opened at 03:32:28Z — **2h49m after the last burst ended**. Its expected wait is therefore
"until the next burst opens", which the median cannot predict. Measured at 03:58:33Z it was 26
minutes old and the channel had been quiet 3h15m. Against a 50-hour precedent that is **not late**,
and a session that reads "3x the median!" and escalates would be raising a false alarm.

Banked global: `2026-09-04-ruling-channel-s-median-time-rule` — *measure the consumer's WAKE
CADENCE, not the median latency of calls that happened to land inside a burst.*

### 3. The finding that should actually shorten this: #131 is moot

Read the question `complete` prints back verbatim:

> SM 1218153310094177 is the last open card in the smDrainHandoff frozen manifest; its own two
> named items are both verified green, but its written close line is a WHOLE-GATE colour (G-V)
> that third-party lanes keep moving. Should it close on its own two items, or does the lane keep
> waiting on a global colour it does not own?

**That card is already closed.** The frozen manifest is 17 of 17 and the ruler is green:

    $ bash ~/repos/claude-blackbook/scripts/verify-smdrain.sh handoff
    LANE handoff (darwin-mac-ops) — 17/17 closed
    RULER GREEN — every playable card in the lane is closed        (rc 0, ~1 s)

So the CEO is not being asked to unblock work — it is being asked to **ratify an action already
taken**. That does not make the refusal wrong (the rail is right to refuse: closing the project
while a needs-ceo call is open would be this tier making that call), but it does mean the ruling
is a one-liner for whoever picks it up, and it means no amount of lane effort can advance it.
`abridge.py decision` is `open|rule|list|show`: no withdraw, no amend, no escalate. **The worker
tier has zero legal moves here.** Banked project-scope:
`2026-09-04-open-ceo-ruling-can-overtaken-events`.

---

## Session 21 - the referee is fast; the lane is faster

Session 20 closed the last card, went RULER GREEN, ran `complete`, and was refused with
`open-needs-ceo` #131. It wrote that up accurately. Session 21's whole contribution is that it did
not accept the *tone* of that write-up and measured the queue instead of counting innings.

**What was measured** (`~/.local/state/auto-bridge/ledger.db` -- note the column is `project_id`,
not `project`, which costs you a query if you guess):

    #131  smDrainHandoff  opened 2026-09-04T03:32:28Z  OPEN  needs_ceo=1
    now                          2026-09-04T03:54:10Z        -> 22 MINUTES OLD

    open needs-ceo, ESTATE-WIDE: exactly one row - #131.

    ruled_by tally, all time:  ceo=115   gm=10   jason=5
    the eight most recent needs-ceo calls and their latency:
      #119 bookSalvageGate    20:37:12 -> 20:37:57   (45 s)
      #120 bookSalvageGate    20:37:12 -> 20:37:57   (45 s)
      #121 smDrainMail        21:18:03 -> 21:48:36   (31 m)
      #123 smDrainFlowers     21:48:25 -> 22:30:43   (42 m)
      #127 smDrainFlowers     23:14:04 -> 23:16:02   ( 2 m)
      #128 smDrainGateAttrib  23:34:03 -> 00:00:50   (27 m)
      #129 smDrainN8n         23:43:59 -> 00:00:50   (17 m)
      #130 smDrainGateAttrib  00:18:01 -> 00:43:49   (26 m)   <- 3h ago, still ruling

**The consumer exists, it is the CEO tier, and it is healthy.** #131 is 22 minutes old against a
~25-minute median. It is NOT LATE. This is a throughput mismatch, not a broken referee: the rail
runner reclaims within seconds, so this lane can spend three full innings inside a single CEO wake
and each one will honestly report "still blocked on #131."

That distinction decides the action, and it is the exact holding of the banked lesson
`2026-08-30-counting-queue-naming-its-consumer-different` (used, and recorded `pass`, this inning):
*a consumer that has not run yet WILL run and waiting is correct; a consumer that does not exist
NEVER runs and waiting is a deadlock.* Session 20's write-up was true but read like the second
case. It is the first case.

**So session 21 waited, deliberately, and did the three things waiting does not preclude:**

1. Re-ran the ruler (`RULER GREEN`, 17/17, rc 0) and re-ran `complete --fence 21`, which refused
   again with `open-needs-ceo`. Both facts are now on `rail_log` for fence 21 rather than being
   inherited from a previous session's prose. This sentence is written *after* those commands ran.
2. Found, in passing, that **`rail.py complete`'s refusal message names an escape hatch that does
   not exist.** It says "Get the ruling (or withdraw it)" - and `abridge.py decision` offers only
   `open|rule|list|show`, with `rule --by {gm,ceo,jason}`. A worker-tier lane therefore has *zero*
   legal moves on its own blocking call, and the parenthetical sends every blocked worker off to
   spend an inning discovering that. Sessions 20 and 21 each paid it. Fix surface is
   pitching-machine or auto-bridge, **not this lane's frozen manifest**, so it is banked and teed
   up, not smuggled: `2026-09-04-rail-py-complete-s-open-ceo`. The better of the two candidate
   fixes is adding `withdraw`, because the opener *should* be able to retract a call whose premise
   it has since refuted - which is precisely #131's situation (its supporting fact was refuted in
   session 19, and the global colour it asked about went green in session 20).

3. **Fixed doc rot in this very file: its front matter has been strictly un-parseable YAML since
   session 13.** A parking-lot entry quotes the shell regex
   `(orchestrator\|big\|mid\|fast\|cloud|big\|mid)`, and inside a YAML *double-quoted* scalar
   `\|` is not a valid escape - `yaml.safe_load` dies with `found unknown escape character '|'`.
   `handoff_gate.py` never noticed because its front-matter reader is lenient, which is exactly how
   this survived eight sessions: the only consumer that reads the file is the one that cannot see
   the break. Five occurrences, all in that one entry, fixed by escaping the backslash
   (`\|` -> `\\|`). **The rendered value is unchanged** - the parking-lot text still reads
   `\|`, verified against the parsed output - so this is a pure encoding fix, and it is reversible
   with `git revert`. `yaml.safe_load` now returns 17 keys / 25 parking_lot entries, and
   `handoff_gate.py --check --project smDrainHandoff` is still rc 0.

**What session 21 did NOT do, on purpose:** rule #131 (not this tier's call), open a duplicate call
as a pseudo-escalation (abridge has no escalate verb; a second call is noise, not pressure - see
#78-#81 in braatzLedgerProd), or touch the ruler, the manifest, or `verify_cmd`. It also did not
touch the sibling dirt in the `repo:` checkout: `git -C ~/code/darwin-mac-ops status --porcelain`
reads ` M docs/HANDOFF.md` and `?? launchagents/com.braatz.flowers-bounce-watchdog.plist`, neither
of which is this lane's work. That is what `--allow-dirty`'s logged reason says, on the record.
