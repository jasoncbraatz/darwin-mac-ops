...
branch: "main"
gh_sha: "f46fa9e448bb37474fe30edc045da25c67184815"
updated: "2026-09-04"
...
lessons_consulted: ["2026-08-17-secret-scanning-gate-reports-coverage-n", "2026-08-07-digit-run-regex-card-number-detector", "2026-08-26-scripts-secret-scan-staged-sh-estate"]
lessons_banked: ["2026-09-04-dead-allocation-table-entry-free-contributes", "2026-09-04-while-ifs-read-r-x-done"]
live_theme: "COVERAGE IS NOT CAPABILITY, AND THE TWO ARE INDISTINGUISHABLE WHILE GREEN. Session 14 said a control can MATCH its subject perfectly and still certify something that stopped being true. Session 15 is the turn under that: a control can be CORRECTLY INSTALLED EVERYWHERE and never have been able to see the thing at all. G-AF counted 114 of 114 repos wired to the secret hook -- a true, verified, useless number, because nobody ever asked whether the needle could match a card number. It could not, and no amount of raising the coverage figure would have revealed that. The only test that separates them is a POSITIVE CONTROL: feed the control a real instance of what it claims to refuse and watch it refuse. A gate that reports N-of-N and has never been shown a specimen is reporting its own installation, not its own competence."
phase: "DRAINING. 17 cards frozen, **14 closed**. Session 15 CLOSED **1217561601836055** (the PAN layer -- the pre-commit hook can now see a card number). TEN closes in ten innings, still obeying the sessions 6-9 rule: take the at-bat whose move is WORKING, never the one whose move is WAITING. Not done -- 3 open, and **two of the three are the known WAITING cards**. Ask again every inning."
gate_passed: false
next_at_bat: "**1217904193313336** — *[near-miss] Registered is not measured: voice-box and mcpMirror boards are registered but never checked*. It is now the ONLY remaining WORKING at-bat: the other two open cards are both WAITING work (1217341652482828 is the ~182s x5 flap probe that has eaten three innings, and 1218153310094177 needs live sibling sessions to be in a particular state). Session 13 already read the adjacent G-AL#board code, so the context is cheap. FIRST FIFTEEN MINUTES: `command grep -n 'G-AL' gate-selfcheck.sh` and find where the board list is enumerated versus where it is CHECKED -- the card's claim is that registration and measurement are two different lists and only one of them is walked. Red-prove the same way sessions 14 and 15 did: write the control that asserts an unmeasured-but-registered board is REPORTED, run it against a `.bak` of the pre-change script, and watch it fail there. **USE `command grep`, not `grep`** -- plain `grep` is shimmed to ugrep in a Claude Code session and returns NOTHING on a file that plainly matches. **DO NOT take 1217341652482828 (the flap) unless you are prepared to start its probe at minute 0 and spend the inning on something else** -- sessions 4, 5 and 6 all shaped an inning around it and none got a verdict."
blockers: []
drift_flags: []
...
fix surface is `gate-selfcheck.sh` / the gate drills / the handoff kit. **14 of 17 closed.**
